...
#define CURRENT_SENSOR_OFFSET 2.5 // ACS712 is a 5V sensor, so its 0A offset is 2.5V
...
SensorConfig sensorMap[] = {
  {0, SENSOR_VOLTAGE, "battery_voltage"},
  {1, SENSOR_CURRENT, "battery_current"},
  {2, SENSOR_VOLTAGE, "teg_voltage"},
  {3, SENSOR_CURRENT, "teg_current"},
  {4, SENSOR_VOLTAGE, "charging_voltage"},
  {5, SENSOR_CURRENT, "charging_current"},
  // Add more sensors easily here
};
 ...
 for (int i = 0; i < NUM_SENSORS; i++) {
  ...
  switch (sensorMap[i].type) {
    case SENSOR_VOLTAGE: {
      ...
      reading = muxVoltage * VOLTAGE_SCALING_FACTOR;
      break;
    }

    case SENSOR_CURRENT: {
      // The ACS712 output is directly read by the ADC, so use muxVoltage
      reading = (muxVoltage - CURRENT_SENSOR_OFFSET) / CURRENT_SENSOR_SENSITIVITY;
      break;
    }
